...
import { useLogStore, useMapStore, useSettings, useBadgeStore } from '@/lib/state';
   ...
   const onToolCall = async (toolCall: LiveServerToolCall) => {
     ...
     try {
       ...
       for (const fc of toolCall.functionCalls) {
        // Badge logic: track itinerary progress
        const { setItineraryProgress } = useBadgeStore.getState();
        if (fc.name === 'frameEstablishingShot') {
          setItineraryProgress({ city: true });
        } else if (fc.name === 'mapsGrounding' && fc.args.query) {
            const query = fc.args.query.toLowerCase();
            if (query.includes('restaurant') || query.includes('food') || query.includes('eat')) {
                setItineraryProgress({ restaurant: true });
            } else if (query.includes('activity') || query.includes('park') || query.includes('museum') || query.includes('do')) {
                setItineraryProgress({ activity: true });
            }
        }
