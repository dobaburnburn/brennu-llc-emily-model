...
import { useLogStore, useUI, useSettings, useBadgeStore } from '@/lib/state';
...
function ControlTray({trayRef}: ControlTrayProps) {
  ...
  useEffect(() => {
    ...
    if (connected && !muted && audioRecorder) {
      ...
      audioRecorder.start().catch(err => {
        ...
        // DOMException names can vary slightly across browsers.
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
          useLogStore.getState().addTurn({
            role: 'system',
            text: 'Microphone access denied. Please enable microphone permissions in your browser settings to use voice input.',
            isFinal: true,
          });
        } else {
          useLogStore.getState().addTurn({
            role: 'system',
            text: `Could not start microphone: ${err.message}`,
            isFinal: true,
          });
        }
        setMuted(true);
  ...
  const handleMicClick = () => {
    ...
    if (!newMutedState && !micUsedRef.current) {
        useBadgeStore.getState().unlockBadge('Communicator');
  ...
  const volumeStyle = { '--volume': `${micVolume * 25}px` } as React.CSSProperties;
